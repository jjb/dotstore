...
1. Keep your dotfiles in a git repository (or any repository, or any directory, maybe synced with dropbox)
   in .dotstore. Here's how I would initiate mine in a new environment:
   ```shell
...
2. Therefore, you can keep any other flat files or directories in .dotstore and they will be ignored.
   For example, [my dot files](http://github.com/jjb/dotfiles) refer to other files in a directory tree in .dotstore.
3. Existing files in your home directory are backed up. Existing symlinks to .dotstore or anywhere else are ignored.
4. You can keep dot directories in .dotstore as well, they will be managed the same as files.
   (I don't have any applications that use dot directories however, so I've only done simple tests for this)
